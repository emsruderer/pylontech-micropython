class PylontechDecode:
    ...
    def decodeSystemParameter(self):
       ...
       if (self.data['ID'] == 0x46) and (len(payload) == 50):
            ...
            i += 4
            self.data['ChargeUpperLimitTemperature'] = self.temperature(payload[i:i+4])
            i += 4
            self.data['ChargeLowerLimitTemperature'] = self.temperature(payload[i:i+4])
            i += 4
            self.data['ChargeCurrentLimit'] = self.moduleCurrent(payload[i:i+4])
            i += 4
            self.data['UpperLimitOfTotalVoltage'] = self.moduleVoltage(payload[i:i+4])
            i += 4
            self.data['LowerLimitOfTotalVoltage'] = self.moduleVoltage(payload[i:i+4])
            i += 4
            self.data['UnderVoltageOfTotalVoltage'] = self.moduleVoltage(payload[i:i+4])
            i += 4
            self.data['DischargeUpperLimitTemperature'] = self.temperature(payload[i:i+4])
            i += 4
            self.data['DischargeLowerLimitTemperature'] = self.temperature(payload[i:i+4])
            i += 4
